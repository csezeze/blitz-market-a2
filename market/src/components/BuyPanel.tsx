...
export function BuyPanel({ product }: { product: Product }) {
  ...
  const disabledReason = useMemo(() => {
    if (!address) return t("connectFirst");
    if (!deployed) return t("contractPending");
    if (!enough) return t("needMore", { amount: product.price - balance, coin: coin.code });
    return "";
  }, [address, deployed, enough, product.price, balance, coin.code, t]);
  ...
  return (
    <aside className="panel rounded-2xl p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-paper/45">{t("payWith")}</div>
        ...
        <div className="text-right font-mono font-black tabular">
          ...
          <div className="text-xs text-paper/45">{t("price")}</div>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-2xl bg-black/25 p-4">
        <div className="mb-3 flex items-center justify-between text-xs font-bold uppercase tracking-widest text-paper/45">
          ...
          <span>{deployed ? t("onChain") : t("pending")}</span>
        </div>
      ...
      >
        {busy ? t("processing") : disabledReason || t("buyNow")}
      </button>
